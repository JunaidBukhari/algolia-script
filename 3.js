...
(() => {
  ...
  function getChildrenOf(parentGroupId) {
    return byParent.get(parentGroupId) ?? [];
  }
  ...
  function clearFilterByGroupId(groupId, state) {
    const field = groupIdToField.get(groupId);
    if (field) delete state[field];
  }
  ...
  function clearAllFilters(runQuery2) {
    ...
    getAllChildren().forEach((child) => {
      if (child.hideUntilRefines) child.childEl.classList.add("is-hidden");
  ...
  async function refreshChildGroup(client, elements, childGroup, parentField, parentValue) {
    if (!parentValue) return;
    await populateDynamicGroup(client, elements, childGroup, {
      facetFilters: [[`${parentField}:${parentValue}`]]
    });
  }
  ...
  function initFilterGroups(runQuery2) {
    ...
    groups.forEach((group) => {
      ...
      if (hideUntil) group.classList.add("is-hidden");
      registerHierarchy({
  ...
  function initBrowsePage(client, config, elements) {
    ...
    on(
      ...
      (evt) => {
        const children = getChildrenOf(evt.parentGroupId);
        if (children.length === 0) return;
        children.forEach((child) => {
          delete FILTER_STATE[child.childField];
          if (child.groupId) clearFilterByGroupId(child.groupId, FILTER_STATE);
          if (evt.newValue) {
            child.childEl.classList.remove("is-hidden");
            refreshChildGroup(
              algoliaClient,
              storedElements,
              child.childEl,
