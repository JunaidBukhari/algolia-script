...
(() => {
  ...
  // src/filters/hierarchical.ts
  init_live_reload();
  var MAX_DEPTH = 5;
  var groupIdToField = /* @__PURE__ */ new Map();
  var groupIdToEl = /* @__PURE__ */ new Map();
  var byParent = /* @__PURE__ */ new Map();
  var byChildEl = /* @__PURE__ */ new WeakMap();
  var allChildren = [];
  var byGroupId = /* @__PURE__ */ new Map();
  function registerGroupIdToField(groupId, field, el) {
    groupIdToField.set(groupId, field);
    groupIdToEl.set(groupId, el);
  }
  function getFieldForGroupId(groupId) {
    return groupIdToField.get(groupId) ?? null;
  }
  function registerHierarchy(entry) {
    const existing = byParent.get(entry.parentGroupId) ?? [];
    existing.push(entry);
    byParent.set(entry.parentGroupId, existing);
    byChildEl.set(entry.childEl, entry);
    byGroupId.set(entry.groupId, entry);
    allChildren.push(entry);
  }
  function getEntryForGroupId(groupId) {
    return byGroupId.get(groupId) ?? null;
  }
  function isParent(groupId) {
    return byParent.has(groupId);
  }
  function getAllChildren() {
    return allChildren.slice();
  }
  function hideGroup(el, hideClass = "is-hidden") {
    el.classList.add(hideClass);
    el.style.setProperty("display", "none", "important");
  }
  function showGroup(el, hideClass = "is-hidden") {
    el.classList.remove(hideClass);
    el.style.removeProperty("display");
  }
  function collectAncestors(groupId) {
    const entries = [];
    const seen = /* @__PURE__ */ new Set();
    let cur = byGroupId.get(groupId);
    while (cur) {
      if (seen.has(cur.groupId)) {
        console.warn(`[wf-algolia] Cycle detected in hierarchy at '${cur.groupId}'`);
        emit("error", new Error(`wf-algolia: hierarchy cycle at '${cur.groupId}'`));
        break;
      }
      seen.add(cur.groupId);
      if (entries.length >= MAX_DEPTH) {
        console.warn(
          `[wf-algolia] collectAncestors exceeded MAX_DEPTH=${MAX_DEPTH} at '${cur.groupId}'`
        );
        break;
      }
      const parentEntry = byGroupId.get(cur.parentGroupId);
  ...
  function collectDescendants(groupId) {
    const result = [];
    const queue = [
      { id: groupId, depthFromRoot: 0 }
    ];
    const seenEls = /* @__PURE__ */ new WeakSet();
    const seenIds = /* @__PURE__ */ new Set([groupId]);
    while (queue.length > 0) {
      const { id, depthFromRoot } = queue.shift();
      if (depthFromRoot >= MAX_DEPTH) continue;
      const children = byParent.get(id) ?? [];
      for (const child of children) {
        if (seenEls.has(child.childEl)) continue;
        seenEls.add(child.childEl);
        result.push(child);
        if (!seenIds.has(child.groupId)) {
          seenIds.add(child.groupId);
          queue.push({ id: child.groupId, depthFromRoot: depthFromRoot + 1 });
        }
      }
    }
    return result;
  }
  function buildAncestorScope(entry, state) {
    const ancestors = collectAncestors(entry.groupId);
    const pairs = [];
    for (const anc of ancestors) {
      const val = firstSelectedValue(state, anc.childField);
      if (val === null) return pairs;
      pairs.push({ field: anc.childField, value: val });
    }
    const parentVal = firstSelectedValue(state, entry.parentField);
    if (parentVal === null) return pairs;
    pairs.push({ field: entry.parentField, value: parentVal });
    return pairs;
  }
  function firstSelectedValue(state, field) {
    const f = state[field];
    if (!f || !f.values || f.values.size === 0) return null;
    return Array.from(f.values)[0];
  }
  ...
  function clearAllFilters(runQuery2) {
    ...
    getAllChildren().forEach((child) => {
      if (child.hideUntilRefines) hideGroup(child.childEl);
      child.childEl.querySelectorAll(".wf-algolia-injected").forEach((el) => el.remove());
    });
  ...
  function setSearchQuery(query, runQuery2) {
    const input = document.querySelector('[wf-algolia-element="browse-search"]') || document.querySelector('[wf-algolia-element="search-input"]');
  ...
  function applyBeforeSearch(params) {
    let result = params;
    for (const mw of middlewares) {
      if (mw.beforeSearch) {
        result = mw.beforeSearch(result) ?? result;
      }
    }
    return result;
  }
  function applyAfterSearch(response) {
    let result = response;
    for (const mw of middlewares) {
      if (mw.afterSearch) {
        result = mw.afterSearch(result) ?? result;
      }
    }
    return result;
  }
  async function runSearchWithMiddleware(params, searchFn) {
    const modifiedParams = applyBeforeSearch(params);
    const response = await searchFn(modifiedParams);
    return applyAfterSearch(response);
  }
  async function runMultiSearchWithMiddleware(client, queries) {
    const modifiedQueries = queries.map((q) => ({
      ...q,
      params: applyBeforeSearch(q.params)
    }));
    const response = await client.multipleQueries(modifiedQueries);
    return {
      ...response,
      results: response.results.map((r) => applyAfterSearch(r))
    };
  }
  ...
  async function populateDynamicGroup(client, elements, group, scope) {
    const facetField = group.getAttribute("wf-algolia-facet");
    const indexName = group.getAttribute("wf-algolia-index") || group.closest('[wf-algolia-element="browse"]')?.getAttribute("wf-algolia-index") || null;
  ...
  async function refreshChildGroup(client, elements, childGroup, scope) {
    if (scope.length === 0) return;
    const facetFilters = scope.map(({ field, value }) => [`${field}:${value}`]);
    await populateDynamicGroup(client, elements, childGroup, { facetFilters });
  }
  ...
  function updateFilterVisibility(facets) {
    document.querySelectorAll('[wf-algolia-element="filter-group"][wf-algolia-field]').forEach((group) => {
      if (group.hasAttribute("wf-algolia-refines")) return;
  ...
  function initFilterGroups(runQuery2) {
    const groups = [...document.querySelectorAll('[wf-algolia-element="filter-group"]')];
    groups.forEach((group) => {
      const field = resolveFilterField(group);
      const gid = group.getAttribute("wf-algolia-group-id");
      if (field && gid) registerGroupIdToField(gid, field, group);
    });
    groups.forEach((group) => {
      const refinesParent = group.getAttribute("wf-algolia-refines");
      if (!refinesParent) return;
      const childField = resolveFilterField(group);
      if (!childField) return;
      const parentField = getFieldForGroupId(refinesParent);
      const hideUntil = group.getAttribute("wf-algolia-hide-until-refines") !== "false";
      if (!parentField) {
        console.warn(
          `[wf-algolia] filter-group refines '${refinesParent}' but no parent group with that id was found; falling back to independent facet.`
        );
        emit("error", new Error(`wf-algolia: unknown refines target '${refinesParent}'`));
        return;
      }
      const parentEntry = getEntryForGroupId(refinesParent);
      const depth = parentEntry ? parentEntry.depth + 1 : 1;
      if (depth > MAX_DEPTH) {
        console.warn(
          `[wf-algolia] Hierarchy chain depth exceeds MAX_DEPTH=${MAX_DEPTH}; skipping group '${group.getAttribute("wf-algolia-group-id") || childField}' (falls back to independent facet).`
        );
        emit("error", new Error(`wf-algolia: chain depth > ${MAX_DEPTH}`));
        return;
      }
      if (hideUntil) hideGroup(group);
      registerHierarchy({
        groupId: group.getAttribute("wf-algolia-group-id") || `__child:${childField}`,
        parentGroupId: refinesParent,
        childEl: group,
        childField,
        parentField,
        hideUntilRefines: hideUntil,
        depth
      });
    });
    groups.forEach((group) => {
      const gid = group.getAttribute("wf-algolia-group-id");
      if (!gid || !isParent(gid)) return;
      const type = group.getAttribute("wf-algolia-type") || "checkbox";
      if (type !== "radio") {
        console.warn(
          `[wf-algolia] Non-leaf filter-group '${gid}' has type='${type}'; hierarchical scoping requires radio. Coercing.`
        );
        emit("error", new Error(`wf-algolia: non-leaf '${gid}' coerced to radio`));
        group.setAttribute("wf-algolia-type", "radio");
      }
    });
    groups.forEach((group) => {
      const field = resolveFilterField(group);
      ...
      const onGroupClick = (e) => {
        ...
        window.setTimeout(() => {
          ...
          const gid = group.getAttribute("wf-algolia-group-id");
          if (gid && isParent(gid)) {
            const selected = FILTER_STATE[field]?.values ?? /* @__PURE__ */ new Set();
            emit("filter:parent-change", {
              parentGroupId: gid,
              parentField: field,
              newValue: selected.size > 0 ? Array.from(selected)[0] : "",
  ...
  function buildFilterParams(state) {
    ...
    Object.entries(state).forEach(([field, entry]) => {
      if (entry.type === "checkbox" || entry.type === "boolean") {
        ...
        if (entry.match === "and") {
          entry.values.forEach((v) => facetFilters.push([`${field}:${escapeFilterValue(v)}`]));
        } else {
          facetFilters.push([...entry.values].map((v) => `${field}:${escapeFilterValue(v)}`));
        }
      }
  ...
  function initShowMore() {
    document.querySelectorAll('[wf-algolia-element="filter-group"]').forEach((group) => {
      ...
      const showMoreBtn = group.querySelector('[wf-algolia-element="filter-show-more"]');
  ...
  function initBrowsePage(client, config, elements) {
    ...
    on(
      ...
      (evt) => {
        const descendants = collectDescendants(evt.parentGroupId);
        if (descendants.length === 0) return;
        descendants.forEach((entry) => {
          delete FILTER_STATE[entry.childField];
          const scope = buildAncestorScope(entry, FILTER_STATE);
          if (scope.length === entry.depth) {
            showGroup(entry.childEl);
            refreshChildGroup(
              algoliaClient,
              storedElements,
              entry.childEl,
              ...
              console.warn("[wf-algolia] refreshChildGroup failed:", err);
              emit("error", err);
            });
          } else {
            if (entry.hideUntilRefines) hideGroup(entry.childEl);
            entry.childEl.querySelectorAll(".wf-algolia-injected").forEach((el) => el.remove());
          }
        });
      }
    );
  ...
  function runSingleIndexQuery(wrapper, template, loader, noResults, append) {
    ...
    runSearchWithMiddleware(
      searchParams,
      (params) => algoliaClient.initIndex(activeIndex).search(browseState.query || "", params)
    ).then((res) => {
      hide(loader);
  ...
  function runMultiIndexQuery(wrapper, template, loader, noResults, append) {
    ...
    runMultiSearchWithMiddleware(algoliaClient, queries).then(({ results }) => {
      hide(loader);
      const allHits = [];
      const mergedFacets = {};
      let totalHits = 0;
      ...
      emit("results", { results, nbHits: totalHits, nbPages: maxPages });
    }).catch((err) => {
      hide(loader);
    ...
    for (const section of recSections) {
      ...
      try {
        ...
        if (model === "related-products") {
          ...
          const res = await recommendClient.getLookingSimilar([
            { indexName, objectID, maxRecommendations: maxRecs }
          ]);
          hits = res.results[0]?.hits || [];
        } else if (model === "trending-items") {
          const res = await recommendClient.getTrendingItems([
            { indexName, maxRecommendations: maxRecs }
          ]);
          hits = res.results[0]?.hits || [];
  ...
  function initAutocomplete(client, config, elements) {
    ...
    autocompleteContainers.forEach((dropdown) => {
      ...
      const input = wrapper?.querySelector('[wf-algolia-element="search-input"]');
      ...
      const search = debounce(
        async (query) => {
          ...
          try {
            const { results } = await runMultiSearchWithMiddleware(client, queries);
      ...
      input.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowDown") {
          ...
          items.forEach((item, i) => item.classList.toggle("wf-algolia-focused", i === activeIndex));
  ...
  function initMergedSearch(client, config, elements) {
    document.querySelectorAll("[wf-algolia-index]").forEach((container) => {
      ...
      const input = container.querySelector('[wf-algolia-element="search-input"]');
  ...
  function initMultiSectionSearch(client, config, elements) {
    ...
    inputs.forEach((input) => {
      ...
      const sections = resultsPanel.querySelectorAll('[wf-algolia-element="section"]');
      const noResults = resultsPanel.querySelector('[wf-algolia-element="no-results"]');
  ...
  function initSearchInstances(client, config, elements) {
    ...
    inputs.forEach((input) => {
      ...
      const search = debounce(async (query) => {
        ...
        show(loader);
        try {
          const res = await runSearchWithMiddleware(
            {
              hitsPerPage: hitsMax,
              clickAnalytics: true,
              attributesToSnippet: ["*:30"]
            },
            (params) => client.initIndex(indexName).search(query, params)
          );
          hide(loader);
          const enrichedHits = res.hits.map((hit) => ({
            ...hit,
          ...
          const countEl = container.querySelector(
            '[wf-algolia-element="results-count"]'
          );
